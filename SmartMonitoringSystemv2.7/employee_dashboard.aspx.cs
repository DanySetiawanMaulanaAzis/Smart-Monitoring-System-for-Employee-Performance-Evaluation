    ...
    public partial class employee_dashboard : System.Web.UI.Page
    {
        ...
        private async Task InsertOrUpdateDailySummary()
        {
            ...
            ORDER BY w.UserId, CAST(w.StartTime AS DATE)";
                ...
                var summaries = new List<(int UserId, DateTime WorkDate, int FinishedToday, int AvgWorkmanship)>();
                ...
                while (await reader.ReadAsync())
                {
                    ...
                    summaries.Add((userId, workDate, finishedToday, avgWorkmanship));
                }

                reader.Close();

                // Hitung TotalWorkmanship kumulatif per user
                var totalWorkmanshipTracker = new Dictionary<int, int>();

                foreach (var summary in summaries)
                {
                    int userId = summary.UserId;

                    if (!totalWorkmanshipTracker.ContainsKey(userId))
                        totalWorkmanshipTracker[userId] = 0;

                    // Tambahkan pekerjaan hari ini ke total sebelumnya
                    totalWorkmanshipTracker[userId] += summary.FinishedToday;

                    int totalWorkmanship = totalWorkmanshipTracker[userId];
                    int efficiencyScore = await GetEfficiencyScore(summary.FinishedToday, summary.AvgWorkmanship, totalWorkmanship);
        ...
        {
            BaseAddress = new Uri("http://127.0.0.1:5000/"), // atau IP lokal
